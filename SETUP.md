...
| 項目 | 状況 |
|---|---|
| 銘柄別ページ（`/ticker/AAPL/` に該当社の全ブリーフ） | ✅ 完了。`/ticker/` に企業一覧、ヘッダーナビ・sitemap にも追加 |
| 記事本文に事実テーブルを追加 | ✅ 完了。front matter から直接生成（LLM非経由・値のブレなし） |
| OG画像（SNSシェア時のサムネイル） | ✅ 完了。`/og-image.png`（1200×630）、全ページ共通 |
| PWA/ブックマーク用の PNG favicon フォールバック | ✅ 完了。`apple-touch-icon.png`・`favicon-32x32.png` |
| **（副産物）ロゴアイコンを v3 に修正** | 実際にレンダリングして確認したところ、それまでの案が「タカ」ではなく「驚いたフクロウ」に見えることが判明。眉を目に被せる／クチバシを1本のフック状に整理して修正。favicon・ヘッダー・ロゴボードすべて反映済み |
| 仕様テンプレート3（IPOウォッチ／S-1要約） | 未着手。次に作るか保留か要判断（Form 4 とは別データソースの本格追加作業） |
